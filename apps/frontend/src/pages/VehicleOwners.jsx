...
function VehicleOwners() {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      await post("/vehicle-owners", { owner_name: name, owner_phone: phone, owner_email: emailInput });
      resetForm();
      setShowModal(false);
      await loadOwners();
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white">Vehicle Owners</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {owners.map((o) => (
              <tr key={o.owner_no} className="border-b border-zinc-800 hover:bg-zinc-800/50">
                <td className="py-2 pr-4 text-white">{o.owner_no}</td>
                <td className="py-2 pr-4 text-white">{o.owner_name}</td>
                <td className="py-2 pr-4 text-zinc-300">{o.owner_phone}</td>
                <td className="py-2 pr-4 text-zinc-300">{o.owner_email}</td>
                {canDelete && (
                  <td className="py-2 pr-4">
                    <button onClick={() => handleDelete(o.owner_no)} className="text-red-400 hover:text-red-300 text-sm">
                      Delete
                    </button>
                  </td>
                )}
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); resetForm(); }} title="Add New Owner">
